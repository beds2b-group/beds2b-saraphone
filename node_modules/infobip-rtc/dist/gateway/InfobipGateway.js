...
import { PortunusHostEvaluator } from "./PortunusHostEvaluator";
export class InfobipGatewayImpl {
    constructor(eventEmitter, logger, accessToken) {
        this.eventEmitter = eventEmitter;
        this.logger = logger;
        this.status = Status.OFFLINE;
        this.accessToken = accessToken;
        this.portunusHostEvaluator = new PortunusHostEvaluator(this.logger);
        this.deviceInfo = this.encodeDeviceInfo();
        this.initRetry();
    }
    async connect(isReconnect = false) {
        if (!this.portunusInstanceHash) {
            this.portunusHost = await this.portunusHostEvaluator.evaluate(this.accessToken);
        }
        this.ws = new WebSocket(this.generatePortunusUrl(), [this.accessToken.jwt, this.deviceInfo]);
    ...
    onOpen() {
        ...
        if (reconnected) {
            this.eventEmitter.once("registered", () => {
    ...
    initRetry() {
        this.retry?.cancel();
        this.retry = new Retry(async (iteration) => {
            this.logger?.info(`Reconnect attempt ${iteration}...`);
            await this.connect(true);
        }, Retry.exponentialDelay(Properties.RECONNECT_INITIAL_DELAY), Properties.RECONNECT_MAX_DELAY, Properties.RECONNECT_MAX).catch(reason => {
    ...
    measureHeartbeatRate() {
        const durationMs = performance.now() - this.heartbeatStartTime;
        if (durationMs > Properties.HEARTBEAT_SLOW_THRESHOLD) {
            this.logger?.warn(`Heartbeat check took too long: ${durationMs} ms`);
        }
    }
}
